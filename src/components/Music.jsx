...
const Music = () => {
  ...
  useEffect(() => {
    try {
      get(`/published_blogs?limit=11&page=${page}&category=music`).then(
        (response) => {
          const sortDateData = response?.data?.data?.sort((a, b) =>
            a.date < b.date ? 1 : -1
          );
          setMusicData(sortDateData);
  ...
  return (
    <div className="flex flex-col gap-5 w-full flex-wrap md:flex-nowrap text-white justify-center items-center p-5 max-w-[1300px] mx-auto my-20 ">
      {musicData?.map((el) => {
        return (
          <div
            key={el.id}
